function [m1_hat, m2_hat] = EstimateMoments(proj_PSWF, weight, total_N, gamma, InputBias)
...
for m=0:max(gamma.ang_idx_2d)
    for k=0:nnz(gamma.ang_idx_2d==0)-1
        current_m2_check(m+1,k+1,:,:) = reshape(m2_hat(m+1 + (k)*(max(gamma.ang_idx_2d)+1),:),...
            max(gamma.ang_idx_2d)+1, nnz(gamma.ang_idx_2d==0) );
%         current_m2_check(m+1,k+1,m+1,k+1) = current_m2_check(m+1,k+1,m+1,k+1) - sigma^2;
    end
end
m1_hat = m1_hat / sum(weight);
m2_hat = current_m2_check / sum(weight) - InputBias;
