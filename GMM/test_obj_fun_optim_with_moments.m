...
%[C_array] = mu2_C_coefs_PSWF_v1(L, P, M);
tt = toc;
...
% [ m1_true_naive ] = FirstMoment_PSWF_naive_fixed(A, B, gamma);  % old code: [ m1_true ] = FirstMoment_PSWF_naive(A, B, gamma);
fprintf('First moment is DONE. \n');
% [Gamma_mat, C_tensor] = preprocessing_mu2_coef(gamma, C_array, P);
% second moment
fprintf('Mu2 calculation...');
tic
% [m2_true] = SecondMoment_PSWF_v1(A, B, gamma, C_array);
[m2_true] = SecondMoment_PSWF_v2(A, B, gamma, C_tensor, Gamma_mat);
% [m2_true] = SecondMoment_PSWF_naive(A, B, gamma);
tt = toc;
fprintf('DONE in about %d seconds \n', round(tt));
...
% imagesc(projs(:,:,4))
% figure
% imagesc(projsNoise(:,:,4))

% Nir
%projs = cryo_project_gaussian('C1_params', gridSize, 1, R); % uniform at the moment

%projs = cryo_project(vol, R);
weight2 = zeros(total_N,1);
rho    = @(x)  wignerD_expansion(B,x); %B_to_Density_func(B,x);
for i=1:total_N
    weight2(i) = rho(rotm2eul(R(:,:,i).'));
end
% R is opposite!
weight = real(weight);
weight(weight<0) = 0;
...
%             inner_absolute_m1(check_counter) = norm(m1_true(:)-current_m1_check(:))^2;
            ...
            unBiass = zeros(size(current_m2_check));
...
%                                                         - sigma^2;
                    unBiass(m+1,k+1,m+1,k+1) = sigma^2;
                end
            end
            current_m2_check = current_m2_check/sum(weight(perm_list(1:i))) - unBiass/i;% - sigma^2;
