function [grad] = get_grad_inplane_PSWF_GMM(x, gamma, C_tensor, Gamma_mat, sign_mat,...
                                     remainder1, remainder2)
% FOR CASE OF INPLANE UNIFORM DISTRIBUTION
...
% back to cell arrays
[A, B] = VecAB_inplane_to_A_B(x, gamma);
...
% B indexing, inplane uniform
ind_B_inplane = zeros(size(vec_B));
start_in = 2;
for p=2:size(B,1)
    curr_B_mat = zeros(2*p-1);
    end_in = numel(curr_B_mat)-1;
    curr_B_mat(:,p) = 1;
    ind_B_inplane(start_in:(start_in+end_in)) = curr_B_mat(:);
    start_in = start_in+end_in+1;
end
...
for m1 = 0:sizeM-1
    ...
    if m1<=max_deg
       ...
       % mu1_grad = mu1_grad + [A_part(mat_ind(:)>0); B_part(2:end)]; 
        mu1_grad = mu1_grad + [A_all; B_part(ind_B_inplane==1)];        
    end
...
% conclusion
dist_part = part2(ind_B_inplane==1);

%ASAFAB
% disp('Asaf! Dont foregt you changed get_grad_inplane_PSWF for first moment only')
% grad = mu1_grad;% + weight*[part1(:); dist_part(:)]; 
grad = mu1_grad + [part1(:); dist_part(:)]; 
